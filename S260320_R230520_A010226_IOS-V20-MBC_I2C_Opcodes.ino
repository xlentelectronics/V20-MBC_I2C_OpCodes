...
S260320-R?????? - HW ref: A250220 (V20-MBC)
...
S260320-R230520-A010226   Feb 2026 - Andy Jackson
                          Added support for I2C LCD display at 0x27. 
                          Derived from the LCD code at https://github.com/xlentelectronics/Z80MBC2-I2C-OpCodes for the Z80-MBC2
                          Added hardware / sw version display on the LCD if present
...
// ------------------------------------------------------------------------------
//
// Hardware definitions for LCD I2C 20 character by 4 lines
//
// ------------------------------------------------------------------------------
...
#include <LiquidCrystal_I2C.h>            // Needed for I2C LCD  // APJ
...
const byte    lcdEnabledAddr = 16;        // Internal EEPROM address for the current lcdEnabled // APJ
...
// LCD I2C variables and initalization
byte          lcdPresent = 0;              // I2C LCD present or not. lcdPresent = 1 LCD is present, lcdPresent = 0 LCD is NOT present.
byte          lcdEnabled = 0;              // lcdEnabled = 0 then disabled. Only Startup info is displayed, no OpCode info. lcdEnabled = 1 enabled, OpCode info is displayed  
byte          lcdLine = 0;                 // Used for the OpCode lcd.setCursor()
LiquidCrystal_I2C lcd( LCD_ADDR, LCD_COL, LCD_ROW );      // LCD I2C address 0x27, type 20 character by 4 lines

// I2C Device OpCode variables and initalization
byte          byteToRec = 1;               // byteToRec is 1 to the number of bytes you want to read from a I2C device 
boolean       multiRead = true;            // multiRead used in Write OpCode 0x90 to set Wire.requestFrom to Stop or Restart condition. True is STOP condition, Flase is Restart condition
...
// ------------------------------------------------------------------------------
  ...
  // Start the communications channels 
  Serial.begin(115200);
  Wire.begin();
  
  // Print some system information
  // Coordinate the values with the lcd.print values below
  Serial.print(F("\r\n\nV20-MBC - A250220\r\nIOS - I/O Subsystem - S260320-R??????\r\n"));
  Serial.println();
  ...
  if (SERIAL_RX_BUFFER_SIZE >= 128)
    Serial.println(F("IOS: Serial : Found extended Rx buffer"));
  else
    Serial.println(F("IOS: Serial : Warning NOT Found extended Rx buffer"));
  ...
  {
  	lcdPresent = 1;   
    Serial.println( F("IOS: I2C LCD : Found") );
    Serial.print( F("IOS: I2C LCD : Display ") );
    Serial.print( LCD_COL );
    Serial.print( F(" char by " ));
  ...
  {
    lcdPresent = 0;
    Serial.println( F("IOS: I2C LCD : NOT Found") ); 
  }
  ...
  if (lcdPresent)
  {
    ...
    // Coordinate the values with the Serial.print versions above
    lcd.print (F("V20-MBC - HW A250220"));
    lcd.setCursor( 0, 2 );
    lcd.print (F("   IOS - S260320    "));
    lcd.setCursor( 0, 3 );
    lcd.print (F("  R230520-A010226   "));

    delay(2500);
    lcd.clear(); 

    // Read LCD Enabled or Disabled
    lcdEnabled = EEPROM.read(lcdEnabledAddr);
    Serial.print(F("IOS: I2C LCD : IO OpCodes ")); 
    if (lcdEnabled)
    {
    ...
    // LCD Only information
    lcd.setCursor( 0, 0 );
    lcd.print(F("V20-MBC System Info"));

    lcd.setCursor( 0, 1 );
    lcd.print(F("V20 clock is "));
  } 
  ...
  if (clockMode)
  {
    ...
    if (lcdPresent) lcd.print(CLOCK_LOW);
  }
  else
  {
    Serial.print(CLOCK_HIGH);
    if (lcdPresent) lcd.print(CLOCK_HIGH);
  }
  Serial.println("MHz");
  if (lcdPresent) lcd.print(F("MHz"));
  ...
  if (moduleGPIO) Serial.println(F("IOS: GPE Option : Available"));
  else Serial.println           (F("IOS: GPE Option : Not fitted"));
  ...
  if (lcdPresent)
  {
    ...
    if (lcdEnabled)
    {
      lcd.print(F("GPE Option : Not fitted"));
    }
    else
    {
      lcd.print(F("GPE Option : Available"));
    }
  } 
  ...
  {
    lcd.setCursor( 0, 3 );
    lcd.print( F("CP/M Autoexec is ") );
  }

  if (autoexecFlag)
  { 
    Serial.println("ON");
    if ( lcdPresent ) lcd.print( "ON" );
  }
  else
  {
    Serial.println("OFF");
    if ( lcdPresent ) lcd.print( "OFF" );
}
  ...
  // or an invalid bootMode code was read from internal EEPROM
  {
    ...
    if (lcdPresent) // APJ
    {
      Serial.print(F(" A: LCD display OpCodes (-> "));
      if (lcdEnabled)
      {
        Serial.println(F("Disabled)"));   // lcdEnabled is enabled, 
      }
      else
      {
        Serial.println(F("Enabled)"));   // lcdEnabled is disabled
      }
    }
    ...
    while (((inChar < minBootChar) || (inChar > maxSelChar))  && (inChar != 'A')) ;
    
    Serial.print( inChar );
    Serial.println("  Ok");

    // Make the selected action for the system paramters choiceV20Z80
    switch (inChar)
    {
      ...
      case 'A':
        ...
        EEPROM.update( lcdEnabledAddr, lcdEnabled );
      break;
  ...
  if ( lcdPresent )
  {
    ...
    if (lcdEnabled)
    {
	    // delay (1500); // Just wait 1.5 seconds to be able to read the LCD info
      lcd.setCursor(0,0);
      lcd.print(F("** V20 I/O OpCode **"));
    }
  }
    ...
    
    {
      ...
      switch (ioAddress)
      {
        ...
        case 1:
          ...
          // LCD I2C OpCodes
          // All OpCode are 1 byte, except OpCode 0x23. 0x23 is 3 bytes.
          // 0x13:        // Clear Screen
          // 0x14:        // Cursor Home
          // 0x15:        // Display OFF
          // 0x16:        // Display ON
...
// Return value: 0 if RTC not present, 1 if found.
{
  ...
  if (Wire.endTransmission() != 0)
  {
      Serial.println("IOS: RTC DS3231 : NOT Found");

     return 0;    // RTC not found
  }
  Serial.print("IOS: RTC DS3231 : Found" );
  Serial.print( "IOS: RTC DS3231 : Current Date / Time ");
  printDateTime(1);
  Serial.println("");
...
void execWriteOpcode()
{
  ...
  if (lcdPresent && lcdEnabled)
  {
    lcd_op_display( ioOpcode, ioData );
  }
  
  // Serial.print ("IOS: OpCode = ");
  ...
  // Execute the requested I/O WRITE Opcode. The 0xFF value is reserved as "No operation".
  {
    ...
    case  0x23: // lcd.Setcursor OpCode
      if (ioByteCnt == 0)
      {
        if ((ioData < 0) || (ioData > 3))
        {
          int i = 1;
          ...
          {
            lcd.clear();
            lcd.print(F("ERROR LCD setCursor!"));
            lcd.setCursor(0,1);
            lcd.print(F("Linenumber "));
            lcd.print(ioData);
            lcd.print(F(" is Out")); 
            delay (2000); 
            lcd.setCursor(0,1);
            lcd.print(F("of Range! Use for   "));
            lcd.setCursor(0,2);
            lcd.print(F("LCD1602 0 or 1 and"));
            lcd.setCursor(0,3);
            lcd.print(F("for LCD2004 0 to 3"));
            delay (3000);
            i++;
          }
            lcd.setCursor(0,1);
    ...
    // I2C J1 interface OpCodes to address a I2C Device
    case 0x30:
    case 0x92:                            // Part of the I2C Device detect OpCode 
      Wire.beginTransmission( ioData );   // I2C Write I2C device address
      break;

    case 0x31:
      //Serial.println("I2C Write command");
      Wire.write(ioData);               // Write a byte to I2C device
      break;        

    case 0x32:
      if (ioData == 1)
      {
        //Serial.println ("I2C restart");
        Wire.endTransmission(false);    // Send I2C restart command to write next byte without
      }                                 // Use this option for multiple Write actions to a I2C device which use an 
      else                              // automatic registerpointer counter to write to multiple registers. 
      {
        //Serial.println("I2C STOP");
        Wire.endTransmission(true);     // Send I2C STOP command and release I2C bus
      }
    break;

    case 0x90:                                // Is part of the READ OpCode 0x90 in the READ section. 
      Wire.beginTransmission(ioData);       // This is the 1 byte READ operation. After 1 byte has been read, a STOP condition is sent                 
      Wire.requestFrom(ioData, 1, true);
      break;                     
    
    case 0x91:                            // Is part of the READ OpCode 0x91 in the READ section.
        switch (ioByteCnt)                // OpCode 0x91 is the READ extended for multi byte read. 
        {
          case 0:                         // How much bytes are we going to read, 1..??
            if (ioData <= 1)
            {
              byteToRec = 1;
            }
          ...
          case 1:                       // Mult Read False or True          
            if (ioData == 1)
            {
              multiRead = false;        // multiRread = false, a restart condition is send to read the next byte form the I2C device. 
            }
            else
            {
              multiRead = true;        // multiRead = true, a Stop condition is send.
            }
          break;

          case 2:                                              
            Wire.beginTransmission(ioData);                     
            Wire.requestFrom(ioData, byteToRec, multiRead);
          break;                       
...
void execReadOpcode()
{
  ...
  // Execute the requested I/O READ Opcode. The 0xFF value is reserved as "No operation".
  {
    ...
    // I2C READ and I2CDetect OpCode 
    case 0x90 ... 0x91:
        ioData = Wire.read();
      break;

    case 0x92:                                  // Used to detect if a I2C Device is present
        ioData = Wire.endTransmission(true);    // A I2C Stop condition is sent. Device can pull down the I2C bus to Acknowledge, returns 0
      break;          
    // End I2C READ
  }
...
// Display the relevant OpCodes on the LCD 
// LcdPresent & lcdEnabled both checked before calling this function

void lcd_op_display( uint8_t opcode, uint8_t data )
// When lcdEnabled is enabled, then IO OpCodes are displayed
{
  ...
  switch( opcode )
  {
    case 0x00:
      lcd.setCursor(0,1);
      lcd.print(F("$00 USERLED "));
      lcd.print( data & 0x01 );
    break;

    // case  0x01: // SERIAL TX:

    case 0x03: // GPIOA Write (GPE Option):
      lcd.setCursor(0,2);
      lcd.print(F("$03 GPIOA Write "));
      lcd.print( data, HEX );
      lcd.print( "  " );
    break;

    case 0x04: // GPIOB Write (GPE Option): 
      lcd.setCursor(0,2);
      lcd.print(F("$04 GPIOB Write "));    
      lcd.print( data, HEX );
      lcd.print( "  " );
    break;

    case 0x05: // IODIRA Write (GPE Option):
      lcd.setCursor(0,2);
      lcd.print(F("$05 IODIRA Write "));
      lcd.print( data, HEX );
      lcd.print( "  " );
    break;

    case 0x06: // IODIRB Write (GPE Option):
      lcd.setCursor(0,2);
      lcd.print(F("$06 IODIRB Write "));
      lcd.print( data, HEX );
      lcd.print( "  " );
    break;

    case 0x07: // GPPUA Write (GPE Option):
      lcd.setCursor(0,2);
      lcd.print(F("$07 GPPUA Write "));
      lcd.print( data, HEX );
      lcd.print( "  " );
    break;

    case 0x08: // GPPUB Write (GPIO Exp. Mod. ):
      lcd.setCursor(0,2);
      lcd.print(F("$08 GPPUB Write "));
      lcd.print( data, HEX );
      lcd.print( "  " );
    break;

    // case  0x09: // DISK EMULATION
    // case  0x0A: // DISK EMULATION // SELTRACK - select the emulated track number (word splitted in 2 bytes in sequence: DATA 0 and DATA 1):
    // case  0x0B: // DISK EMULATION // SELSECT - select the emulated sector number (binary):
    // case  0x0C: // DISK EMULATION // WRITESECT - write 512 data bytes sequentially into the current emulated disk/track/sector:

    //case 0x11:
    //  lcd.setCursor(0,1);
    //  lcd.print(F("* GPIO in SPP MODE *"));
    //  lcd.setCursor(0,2);
    //  lcd.print(F("Printer is selected!"));
    //  lcd.setCursor(0,3);
    //  lcd.print(F("                    "));
    //break;
    ...
    case 0x80: // USER KEY
      lcd.setCursor(0,3);
      lcd.print(F("$80 USERKEY Read    "));
    break;

    case 0x81: // GPIOA Read (GPE Option):
      lcd.setCursor(0,3);
      lcd.print(F("$81 GPIOA Read      "));
    break;

    case 0x82:
      lcd.setCursor(0,3);
      lcd.print(F("$82 GPIOB Read      "));    
    break;
  }
